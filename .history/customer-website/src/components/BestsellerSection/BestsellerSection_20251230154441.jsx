...
const items = [
  ...
  { title: 'SP. Dana Gud Ladu', subtitle: 'Traditional Sweet Delight', price: '₹280', rating: '5.0', img: 'dana_ladu.png' },
  { title: 'Pineapple Chocolate Chikki', subtitle: 'Exotic Flavor Fusion', price: '₹390', rating: '5.0', img: 'mango_chikki.png' },
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="mt-4 text-[15px] text-[var(--text-muted)] max-w-xl mx-auto leading-relaxed">
            Our most loved chikkis and special combos, crafted with premium ingredients and irresistible crunch.
          </p>
        </motion.div>

        {/* Desktop Carousel */}
        <div
          className="hidden md:flex relative rounded-[60px] px-14 py-20 bg-cover bg-center bg-no-repeat items-center justify-center"
          style={{ backgroundImage: "url('Group 57.png')" }}
        >
          <button
            onClick={prevSlide}
            className="absolute -left-3 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-white shadow flex items-center justify-center hover:scale-110 transition"
          >
            ...
            {getVisibleItems(4).map((item, i) => (
              ...
              >
                <motion.div
                  ...
                  className="absolute -top-14 left-1/2 -translate-x-1/2 w-24 h-24 bg-white rounded-full shadow-md flex items-center justify-center"
                >
                  <img src={item.img} alt={item.title} className="w-20 h-20 rounded-full object-cover" />
                </motion.div>
          ...
          <button
            onClick={nextSlide}
            className="absolute -right-3 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-white shadow flex items-center justify-center hover:scale-110 transition"
          >
        ...
        <div className="md:hidden relative py-10 px-4">
          <button
            onClick={prevSlide}
            className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow flex items-center justify-center"
          >
          ...
          <div className="flex gap-4 overflow-hidden">
            {getVisibleItems(1).map((item, i) => (
              <div key={i} className="min-w-[260px] mx-auto bg-white rounded-2xl p-4 shadow-md">
                <img src={item.img} className="w-24 h-24 mx-auto rounded-full mb-4" />
                <h3 className="font-semibold text-center">{item.title}</h3>
                <p className="text-xs text-center text-[var(--text-muted)]">{item.subtitle}</p>
          ...
          <button
            onClick={nextSlide}
            className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow flex items-center justify-center"
          >
