...
export default function BestSellersInteractive() {
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        >
          ...
          <p className="mt-3 text-[15px] text-[var(--text-muted)] max-w-xl mx-auto">
            Discover customer favorites and value-packed chikki combos made with love.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {products.map((item, i) => (
            <motion.div
              key={i}
              whileHover={{ y: -8 }}
              className="bg-white rounded-3xl p-5 shadow-[0_18px_45px_rgba(0,0,0,0.12)] relative group"
            >
              {/* Tag */}
              ...
              <motion.button
                ...
                className={`absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center z-10 shadow
                  ${wishlist[i]
              ...
              <div className="w-full h-48 flex items-center justify-center mb-4">
                ...
                <img
                  src={item.hoverImg}
                  alt="hover"
                  className="max-h-full max-w-full object-contain absolute opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                />
              </div>
