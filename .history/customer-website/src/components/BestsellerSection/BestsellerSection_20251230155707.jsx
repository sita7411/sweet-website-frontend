    ...
    <section className="py-20">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {products.map((item, i) => (
            <div key={i} className="bg-white rounded-3xl p-5 shadow-[0_18px_45px_rgba(0,0,0,0.12)] relative group">

              {/* Tag */}
              <span className="absolute top-4 left-4 bg-[var(--primary)] text-white text-[11px] px-3 py-1 rounded-full">
                {item.tag}
              </span>
              ...
              <button
                ...
                ${wishlist[i] ? 'bg-red-500 text-white' : 'bg-white text-red-500'}`}
              >
              ...
              <div className="relative h-44 mb-4 flex items-center justify-center">
                <img src={item.img} className="absolute w-full h-full object-contain transition group-hover:opacity-0" />
                <img src={item.hoverImg} className="absolute w-full h-full object-contain opacity-0 group-hover:opacity-100 transition" />
              </div>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                {!qty[i] ? (
                  <button
                    ...
                    className="px-5 py-2 rounded-full text-xs font-semibold bg-[var(--primary)] text-white"
                  >
                    Add
                  </button>
                ) : (
                  <div className="flex items-center gap-3 bg-[var(--bg-main)] px-3 py-1 rounded-full">
                    <button onClick={() => decreaseQty(i)} className="text-lg font-bold">−</button>
                    <span className="text-sm font-semibold">{qty[i]}</span>
                    <button onClick={() => increaseQty(i)} className="text-lg font-bold">+</button>
                  </div>
                )}
              </div>
