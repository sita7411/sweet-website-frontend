  ...
  return (
    <footer className="bg-[var(--bg-main)] text-[var(--text-main)]">
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
        ...
        {/* Brand with Logo */}
        <div>
          <img src="/Logo_Marvel.png" alt="ChikkiWala Logo" className="h-14 mb-4" />
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {latestProducts.map((item, i) => (
              <div
                key={i}
                className="flex items-start gap-3 bg-[var(--bg-soft)] rounded-lg p-3 hover:shadow-lg hover:scale-105 transition transform cursor-pointer"
              >
                <img
                  src={item.img}
                  alt={item.name}
