...
export default function HeroBanner() {
  ...
  return (
    <div className="relative min-h-screen bg-gray-50 flex items-center justify-center px-6 lg:px-20">
      ...
      {/* Center Food Image with Rings */}
      <div className="relative mx-10 flex items-center justify-center">
        {/* Rings */}
        <div className="absolute w-80 h-80 sm:w-96 sm:h-96 rounded-full border-4 border-yellow-200 animate-pulse opacity-40"></div>
        <div className="absolute w-64 h-64 sm:w-80 sm:h-80 rounded-full border-4 border-yellow-100 animate-pulse opacity-30"></div>

        {/* Food Image */}
        <motion.div
          key={activeTab}
          className="w-64 h-64 sm:w-80 sm:h-80 rounded-full overflow-hidden shadow-2xl border-8 border-white relative z-10"
