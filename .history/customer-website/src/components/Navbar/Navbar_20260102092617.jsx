...
export default function Navbar() {
  ...
  /* ---------- CLOSE MOBILE SEARCH ON ROUTE CHANGE OR OUTSIDE CLICK ---------- */
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 h-[90px] flex items-center justify-between">
        ...
        <div className={`flex-1 mx-4 lg:mx-10 ${mobileSearchOpen ? "block" : "hidden lg:flex"}`}>
          ...
          {isLoggedIn ? (
            <div className="relative hidden sm:block group">
              ...
              <div className="absolute right-0 mt-3 w-56 bg-[var(--bg-card)] rounded-xl shadow-lg border border-gray-200 opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50">
                <AccountItem as={Link} to="/orders" label="My Orders" />
                <AccountItem as={Link} to="/profile" label="Profile" />
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="px-6 py-6 space-y-4">
              ...
              <NavLink to="/collections" mobile>Collections</NavLink>
              <NavLink to="/offers" mobile>Offers</NavLink>
              <NavLink to="/about" mobile>Our Story</NavLink>
        ...
        {cartOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 h-full w-full sm:w-[380px] md:w-[420px] bg-[var(--bg-card)] shadow-2xl z-[9999] flex flex-col"
            >
                ...
                {cartItems.map((item) => (
                  <div key={item.id} className="flex gap-4 pb-4 border-b last:border-b-0">
                    <img
                      src={item.img}
                      alt={item.title}
                      className="w-20 h-20 rounded-xl bg-[var(--bg-soft)] object-contain"
                    />
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex items-center border rounded-full overflow-hidden">
                          <button
                            ...
                            className="px-3 py-1 hover:bg-[var(--primary)] hover:text-white transition"
                          >
                            −
                          </button>
              ...
              <div className="border-t px-5 py-4 space-y-3">
                ...
                <div className="bg-[var(--bg-soft)] rounded-xl p-3 space-y-2">
                  ...
                  <div className="flex items-start gap-2 text-sm text-[var(--text-muted)]">
                    <Clock size={16} className="mt-0.5 text-[var(--secondary)]" />
                    <span>Estimated delivery in <b>3–5 business days</b></span>
                  </div>
                  <div className="flex items-start gap-2 text-sm text-[var(--text-muted)]">
                    <ShieldCheck size={16} className="mt-0.5 text-[var(--secondary)]" />
                    <span>Free delivery on orders above <b>₹499</b></span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full border border-[var(--primary)] text-[var(--primary)] py-2 rounded-xl text-sm hover:bg-[var(--primary)] hover:text-white transition"
                >
                  View Full Cart
                </button>
...
const IconBtn = ({ children, className = "", onClick }) => (
  <button
    ...
    onClick={onClick}
    className={`
      relative p-2.5 rounded-full bg-[var(--bg-soft)] cursor-pointer group
      hover:bg-[var(--primary)] transition-all duration-300 flex items-center justify-center ${className}
    `}
  >
...
const AccountItem = ({ label, as: Component = "button", to, ...props }) => (
  <Component
    {...(Component === Link ? { to } : {})}
    className="block w-full text-left px-5 py-2 hover:bg-[var(--bg-soft)] text-[var(--text-main)]"
    {...props}
  >
    {label}
  </Component>
