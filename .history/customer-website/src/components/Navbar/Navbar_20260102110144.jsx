  ...
  return (
    <header className={`sticky top-0 z-50 transition-all ${scrolled ? "backdrop-blur bg-[var(--bg-main)]/90 shadow-md" : "bg-[var(--bg-main)]"}`}>
      ...
      <AnimatePresence>
        {cartOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.45 }} exit={{ opacity: 0 }} onClick={() => setCartOpen(false)}   className="fixed right-0 top-0 h-full w-full sm:w-[380px] md:w-[420px] bg-[var(--bg-card)] shadow-2xl z-[9999] flex flex-col" />
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "tween", duration: 0.35 }} className="fixed right-0 top-0 h-full w-full sm:w-[380px] md:w-[420px] bg-[var(--bg-card)] shadow-2xl z-[9999] flex flex-col">
              {/* HEADER */}
