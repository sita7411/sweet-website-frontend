...
<AnimatePresence>
  {cartOpen && (
    <>
      ...
      {/* Drawer - MAIN FIX YAHAN HAI */}
      <motion.div
        initial={{ x: "100%" }}
        animate={{ x: 0 }}           // ← "0%" की बजाय 0 (number) use करो
        exit={{ x: "100%" }}
        transition={{ type: "tween", duration: 0.35 }}
        // layout prop हटाओ ← ये problem create कर रहा था
        className="fixed inset-y-0 right-0 w-full max-w-md bg-[var(--bg-card)] shadow-2xl z-[9999] flex flex-col overflow-hidden"
        style={{ transform: "translateX(0%)" }}  // ← ये line add करो (important!)
      >
              {/* HEADER */}
