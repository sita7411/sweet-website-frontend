'use client';

import { motion } from 'framer-motion';
import {
  ChevronLeftIcon,
  ChevronRightIcon,
  HeartIcon,
} from '@heroicons/react/24/solid';

const items = [
  {
    title: 'Classic Nut Chikki',
    subtitle: 'Jaggery & Peanut Blend',
    price: '₹199',
    rating: '5.0',
    img: 'pista.png',
  },
  {
    title: 'Strawberry Delight',
    subtitle: 'Fruity Crunch Chikki',
    price: '₹149',
    rating: '5.0',
    img: 'stwabarry_chikki.png',
  },
  {
    title: 'Mango Fusion',
    subtitle: 'Seasonal Mango Flavor',
    price: '₹179',
    rating: '5.0',
    img: 'mango_chikki.png',
  },
  {
    title: 'Chocolate Crunch',
    subtitle: 'Rich Cocoa Coated',
    price: '₹199',
    rating: '5.0',
    img: 'choclate.png',
  },
];

export default function PopularItems() {
  return (
    <section className="py-28 bg-[var(--bg-main)]">
      <div className="max-w-6xl mx-auto">

        {/* ===== Heading ===== */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          viewport={{ once: true }}
          className="text-center mb-20"
        >
          <h2 className="text-[42px] font-extrabold text-[var(--text-main)]">
            Our Popular Picks
          </h2>
          <p className="mt-4 text-[15px] text-[var(--text-muted)] max-w-xl mx-auto">
            Handcrafted chikki made with premium ingredients, loved for its
            authentic taste and perfect crunch.
          </p>
        </motion.div>

        {/* ===== Slider Container ===== */}
        <div className="relative bg-[var(--bg-soft)] rounded-[60px] px-24 py-20 overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,0.12)]">

          {/* ================= LEFT CURVE'use client';

import { motion } from 'framer-motion';
import {
  ChevronLeftIcon,
  ChevronRightIcon,
  HeartIcon,
} from '@heroicons/react/24/solid';

const items = [
  {
    title: 'Classic Nut Chikki',
    subtitle: 'Jaggery & Peanut Blend',
    price: '₹199',
    rating: '5.0',
    img: 'pista.png',
  },
  {
    title: 'Strawberry Delight',
    subtitle: 'Fruity Crunch Chikki',
    price: '₹149',
    rating: '5.0',
    img: 'stwabarry_chikki.png',
  },
  {
    title: 'Mango Fusion',
    subtitle: 'Seasonal Mango Flavor',
    price: '₹179',
    rating: '5.0',
    img: 'mango_chikki.png',
  },
  {
    title: 'Chocolate Crunch',
    subtitle: 'Rich Cocoa Coated',
    price: '₹199',
    rating: '5.0',
    img: 'choclate.png',
  },
];

export default function PopularItems() {
  return (
    <section className="py-28 bg-[var(--bg-main)]">
      <div className="max-w-6xl mx-auto">

        {/* Heading */}
        <h2 className="text-center text-[42px] font-extrabold mb-20">
          Our Popular Picks
        </h2>

        {/* Container */}
        <div className="relative bg-[#fde9cf] rounded-[60px] px-24 py-20 overflow-hidden">

          {/* ================= LEFT SHAPE ================= */}
          <svg
            className="absolute left-0 top-0 h-full w-32"
            viewBox="0 0 120 600"
            preserveAspectRatio="none"
          >
            <path
              d="
                M120 0
                L60 0
                C10 150, 10 450, 60 600
                L120 600
                Z
              "
              fill="#fde9cf"
            />
          </svg>

          {/* Left Arrow */}
          <button className="absolute left-10 top-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full shadow-lg flex items-center justify-center">
            <ChevronLeftIcon className="w-6 h-6" />
          </button>

          {/* ================= RIGHT SHAPE ================= */}
          <svg
            className="absolute right-0 top-0 h-full w-32"
            viewBox="0 0 120 600"
            preserveAspectRatio="none"
          >
            <path
              d="
                M0 0
                L60 0
                C110 150, 110 450, 60 600
                L0 600
                Z
              "
              fill="#fde9cf"
            />
          </svg>

          {/* Right Arrow */}
          <button className="absolute right-10 top-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full shadow-lg flex items-center justify-center">
            <ChevronRightIcon className="w-6 h-6" />
          </button>

          {/* Cards */}
          <div className="relative z-10 grid grid-cols-4 gap-10">
            {items.map((item, i) => (
              <motion.div
                key={i}
                whileHover={{ y: -10 }}
                className="bg-white rounded-[28px] px-6 pt-20 pb-7 shadow-lg relative"
              >
                <div className="absolute -top-14 left-1/2 -translate-x-1/2 w-28 h-28 bg-[#fff7ef] rounded-full flex items-center justify-center shadow">
                  <img src={item.img} className="w-24 h-24 rounded-full" />
                </div>

                <h3 className="font-semibold">{item.title}</h3>
                <p className="text-xs text-gray-500">{item.subtitle}</p>
                <p className="text-xl font-bold mt-3">{item.price}</p>

                <div className="flex justify-between mt-6">
                  <button className="bg-orange-200 px-5 py-1.5 rounded-full text-xs font-semibold">
                    Order Now
                  </button>
                  <div className="flex items-center gap-2">
                    <HeartIcon className="w-4 h-4 text-red-500" />
                    <span className="text-xs">★ {item.rating}</span>
                  </div>
                </div>
              </motion.div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
 ================= */}
          <div className="absolute left-0 top-0 h-full w-28 pointer-events-none">
            <div
              className="absolute inset-0"
              style={{
                background: `
                  radial-gradient(circle at right center, transparent 58%, #fde9cf 60%),
                  linear-gradient(to right, #fde9cf, #fde9cf)
                `,
              }}
            />
          </div>

          {/* LEFT ARROW */}
          <button
            className="
              absolute left-10 top-1/2 -translate-y-1/2
              w-14 h-14 rounded-full bg-white
              shadow-lg flex items-center justify-center
              hover:scale-110 transition
            "
          >
            <ChevronLeftIcon className="w-6 h-6 text-black" />
          </button>

          {/* ================= RIGHT CURVE ================= */}
          <div className="absolute right-0 top-0 h-full w-28 pointer-events-none">
            <div
              className="absolute inset-0"
              style={{
                background: `
                  radial-gradient(circle at left center, transparent 58%, #fde9cf 60%),
                  linear-gradient(to left, #fde9cf, #fde9cf)
                `,
              }}
            />
          </div>

          {/* RIGHT ARROW */}
          <button
            className="
              absolute right-10 top-1/2 -translate-y-1/2
              w-14 h-14 rounded-full bg-white
              shadow-lg flex items-center justify-center
              hover:scale-110 transition
            "
          >
            <ChevronRightIcon className="w-6 h-6 text-black" />
          </button>

          {/* ================= CARDS ================= */}
          <div className="grid grid-cols-4 gap-10 relative z-10">
            {items.map((item, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                whileHover={{ y: -10 }}
                transition={{ duration: 0.5, delay: index * 0.08 }}
                viewport={{ once: true }}
                className="relative bg-white rounded-[28px] px-6 pt-20 pb-7 shadow-[0_18px_45px_rgba(0,0,0,0.12)]"
              >
                {/* Image */}
                <motion.div
                  whileHover={{ scale: 1.1, rotate: 4 }}
                  transition={{ type: 'spring', stiffness: 200 }}
                  className="absolute -top-14 left-1/2 -translate-x-1/2 w-28 h-28 bg-[var(--bg-card)] rounded-full shadow-md flex items-center justify-center"
                >
                  <img
                    src={item.img}
                    alt={item.title}
                    className="w-24 h-24 rounded-full object-cover"
                  />
                </motion.div>

                {/* Content */}
                <h3 className="text-[15px] font-semibold text-[var(--text-main)]">
                  {item.title}
                </h3>
                <p className="text-xs text-[var(--text-muted)] mt-1">
                  {item.subtitle}
                </p>

                <p className="text-[22px] font-bold text-[var(--secondary)] mt-3">
                  {item.price}
                </p>

                {/* Actions */}
                <div className="flex items-center justify-between mt-6 -ml-2">
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="bg-[var(--accent)] text-[var(--secondary)] text-xs px-5 py-1.5 rounded-full font-semibold shadow"
                  >
                    Order Now
                  </motion.button>

                  <div className="flex items-center gap-2">
                    <motion.button
                      whileHover={{ scale: 1.15 }}
                      className="w-8 h-8 bg-[var(--bg-card)] rounded-full flex items-center justify-center shadow"
                    >
                      <HeartIcon className="w-4 h-4 text-[var(--primary)]" />
                    </motion.button>
                    <span className="text-xs font-medium text-[var(--text-muted)]">
                      ★ {item.rating}
                    </span>
                  </div>
                </div>
              </motion.div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
