  ...
  return (
    <section className="py-28">
      <div className="max-w-6xl mx-auto">
        ...
        {/* ===== Slider Container ===== */}
        <div
          className="relative rounded-[60px] px-10 py-18 bg-cover bg-center bg-no-repeat"
          style={{ backgroundImage: "url('Group 57.png')" }}
        >
          {/* Left Arrow */}
          <button
            onClick={() => scroll('left')}
            className="absolute -left-7 top-1/2 -translate-y-1/2 w-19 h-19 rounded-full bg-white shadow flex items-center justify-center hover:scale-110 transition"
          >
          ...
          <motion.div
            ref={sliderRef}
            className="flex gap-10 overflow-x-auto scrollbar-hide py-5"
            drag="x"
          ...
          >
            {items.map((item, index) => (
              <motion.div
                ...
                className="relative flex-none w-72 bg-white rounded-[28px] px-6 pt-20 pb-7 shadow-[0_18px_45px_rgba(0,0,0,0.12)]"
              >
                {/* Image */}
                <motion.div
                  whileHover={{ scale: 1.1, rotate: 4 }}
                  transition={{ type: 'spring', stiffness: 200 }}
                  className="absolute -top-14 left-1/2 -translate-x-1/2 w-28 h-28 bg-[var(--bg-card)] rounded-full shadow-md flex items-center justify-center"
                >
                  <img
                    src={item.img}
                    alt={item.title}
                ...
                <p className="text-[22px] font-bold text-[var(--secondary)] mt-3">
                  {item.price}
                </p>
                ...
                <div className="flex items-center justify-between mt-2 -ml-2">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="bg-[var(--accent)] text-[var(--secondary)] text-xs px-5 py-1.5 rounded-full font-semibold shadow hover:brightness-105 transition"
                  >
                    Order Now
                  </motion.button>
          ...
          <button
            ...
            className="absolute -right-6 top-1/2 -translate-y-1/2 w-19 h-19 rounded-full bg-white shadow flex items-center justify-center hover:scale-110 transition"
          >
