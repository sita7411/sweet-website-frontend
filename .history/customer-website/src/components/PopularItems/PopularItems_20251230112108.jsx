...
export default function PopularItems() {
  ...
  const [cart, setCart] = useState(Array(itemsData.length).fill(false)); // track added items
  ...
  const toggleCart = (i) => {
    const newCart = [...cart];
    newCart[i] = !newCart[i]; // toggle added/removed
    setCart(newCart);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-4 gap-10 mt-9">
            {itemsData
              .slice(index, index + 4)
              .concat(index + 4 > itemsData.length ? itemsData.slice(0, (index + 4) - itemsData.length) : [])
              .map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <motion.button
                        ...
                        className={`w-8 h-8 rounded-full flex items-center justify-center shadow ${
                          cart[itemIndex] ? 'bg-[var(--accent)]' : 'bg-[var(--bg-card)]'
                        }`}
                      >
                        <HeartIcon
                          className={`w-4 h-4 ${
                            cart[itemIndex] ? 'text-white' : 'text-[var(--primary)]'
                          }`}
                        />
                      </motion.button>
                    </div>
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`
                        w-full text-xs px-5 py-1.5 mt-3 rounded-full font-semibold shadow transition
                        ${cart[itemIndex]
