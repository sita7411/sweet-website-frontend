  ...
  return (
    <section className="py-10">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-[42px] font-extrabold text-[var(--text-main)]">
            Our Popular Picks
          </h2>
          <p className="mt-4 text-sm sm:text-[15px] text-[var(--text-muted)] max-w-xl mx-auto leading-relaxed">
        ...
        {/* Cards Container */}
        <div
          className="relative rounded-[60px] px-6 py-12 sm:px-13 sm:py-18 -mt-8 bg-cover bg-center bg-no-repeat flex items-center justify-center"
          style={{ backgroundImage: "url('Group 57.png')" }}
        >
          {/* Left Arrow */}
          <button
            onClick={prevSlide}
            className="absolute left-2 sm:-left-3 top-1/2 -translate-y-1/2 w-12 sm:w-19 h-12 sm:h-19 rounded-full bg-white shadow flex items-center justify-center hover:scale-110 transition"
          >
            <ChevronLeftIcon className="w-6 sm:w-8 h-6 sm:h-8 text-[var(--text-main)]" />
          </button>
          ...
          <div className="flex flex-wrap justify-center gap-6 sm:gap-10 mt-9">
            {getVisibleItems().map((item, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="relative bg-white rounded-[28px] px-4 sm:px-6 pt-24 sm:pt-28 pb-6 sm:pb-7 shadow-[0_18px_45px_rgba(0,0,0,0.12)] w-full sm:w-[48%] md:w-[23%]"
              >
                {/* Image */}
                <motion.div
                  whileHover={{ scale: 1.1, rotate: 4 }}
                  transition={{ type: 'spring', stiffness: 200 }}
                  className="absolute -top-12 sm:-top-14 left-1/2 -translate-x-1/2 w-20 sm:w-25 h-20 sm:h-25 bg-[var(--bg-card)] rounded-full shadow-md flex items-center justify-center"
                >
                  <img
                    src={item.img}
                    alt={item.title}
                ...
                {/* Content */}
                <h3 className="text-sm sm:text-[15px] -mt-10 sm:-mt-12 font-semibold text-[var(--text-main)]">
                  {item.title}
                </h3>
                ...
                <div className="flex items-center justify-between mt-3">
                  <p className="text-lg sm:text-[22px] font-bold text-[var(--secondary)]">{item.price}</p>
                  <motion.button
                ...
                <div className="flex items-center justify-between mt-4 -ml-2 gap-3">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={`flex items-center gap-2 text-xs px-4 sm:px-5 py-1.5 rounded-full font-semibold shadow transition ${
                      addedItems[i]
                        ? 'bg-[var(--accent)] text-white'
          ...
          <button
            onClick={nextSlide}
            className="absolute right-2 sm:-right-3 top-1/2 -translate-y-1/2 w-12 sm:w-19 h-12 sm:h-19 rounded-full bg-white shadow flex items-center justify-center hover:scale-110 transition"
          >
            <ChevronRightIcon className="w-6 sm:w-8 h-6 sm:h-8 text-[var(--text-main)]" />
          </button>
        </div>
      </div>
    </section>
