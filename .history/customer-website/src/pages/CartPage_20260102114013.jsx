...
const CartPage = () => {
  ...
  const [isProcessing, setIsProcessing] = useState(false); // Checkout processing state
  ...
  const handleCheckout = () => {
    const inStockItems = cart.filter((item) => item.stock === 'In stock');
    if (inStockItems.length === 0) return; // do nothing if no stock

    setIsProcessing(true);

    // Simulate checkout process (2 seconds)
    setTimeout(() => {
            ...
            <div className="w-full lg:w-[320px] bg-[var(--bg-card)] p-6 rounded-lg shadow flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-2">
                ...
                <button
                  ...
                  className="w-full bg-[var(--secondary)] text-white py-3 px-3 whitespace-nowrap rounded hover:bg-[var(--primary)] transition font-semibold"
                >
                  Add All to Checkout
                </button>
              </div>
