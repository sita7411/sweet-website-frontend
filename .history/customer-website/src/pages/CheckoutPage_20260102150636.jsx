              ...
              <div>
                ...
                {payment === "card" && (
                  <div className="mt-6 space-y-5">
                    <div className="relative bg-gradient-to-r from-[#6b3f26] to-[#8b5e3c] rounded-xl p-6 text-white shadow-md">
                      <p className="text-lg font-bold">{form.cardName || "Cardholder Name"}</p>
                      <p className="mt-2 tracking-widest">{maskedCardNumber}</p>
          ...
          <div className="order-1 lg:order-2 lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8 sticky top-6">
              ...
              <button
                onClick={handleOrder}
                disabled={loading}
                className="w-full mt-6 py-4 rounded-xl font-bold text-white text-lg bg-[#6b3f26] hover:bg-[#8b5e3c] transition duration-200 shadow-md flex justify-center items-center gap-2"
              >
                {loading ? <span className="animate-spin">⏳</span> : "Place Order"}
                {loading ? "Processing..." : ""}
              </button>

              <div className="flex flex-col items-center justify-center mt-4 text-gray-600 text-sm gap-1">
...
function CustomSelect({ label, required, options = [], value, onChange }) {
  ...
  return (
    <div className="w-full" ref={ref}>
      ...
      >
        <div className="w-full px-4 py-3 border border-gray-300 rounded-2xl bg-[var(--bg-card)] text-[var(--text-main)] flex justify-between items-center shadow-sm hover:border-[var(--secondary)] hover:shadow-md transition">
          <span>{value || `Select ${label}`}</span>
          <svg
            className={`w-5 h-5 text-[var(--secondary)] transition-transform duration-200 ${open ? "rotate-180" : ""}`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
        ...
        {open && (
          <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-2xl max-h-60 overflow-auto shadow-lg">
            {options.map((opt) => (
              <li
                key={opt}
                onClick={() => {
                ...
                className="px-4 py-3 hover:bg-[var(--secondary)] hover:text-white transition cursor-pointer"
              >
                {opt}
              </li>
            ))}
