...
<div className="mt-3 relative">
  <select
    ...
    onChange={(e) =>
      setSelectedVariant((p) => ({ ...p, [i]: e.target.value }))
    }
    className="w-full px-4 py-2 pr-10 border border-gray-300 rounded-2xl bg-white text-sm text-[var(--text-main)] font-medium
      shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)] transition
      hover:border-[var(--accent)] cursor-pointer appearance-none"
  >
    {Object.keys(item.variants).map((weight, idx) => (
  ...
  <ChevronDownIcon className="w-5 h-5 absolute top-1/2 right-3 -translate-y-1/2 text-[var(--text-muted)] pointer-events-none" />
</div>
