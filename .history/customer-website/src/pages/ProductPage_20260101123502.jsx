      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            exit={{ opacity: 0, y: -6 }}
            className="absolute z-20 w-full mt-1 bg-white border border-black/10 rounded-xl overflow-hidden shadow-md"
          >
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-20 bg-[var(--bg-soft)] text-center">
        <h1 className="text-4xl md:text-5xl font-semibold text-[var(--text-main)]">
          Marvel Crunch Chikki
        </h1>
        <p className="mt-4 text-sm md:text-base text-[var(--text-muted)]">
          Crafted with tradition, perfected for taste.
        </p>
      </section>
            ...
            {filteredProducts.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white border border-black/5 rounded-2xl p-6 transition"
                >
                  {/* Wishlist */}
                  <button
                    onClick={() => setWishlist(p => ({ ...p, [i]: !p[i] }))}
                    className={`absolute mt-2 mr-2 w-9 h-9 rounded-full flex items-center justify-center border ${
                  ...
                  {/* Image */}
                  <div className="h-44 bg-[var(--bg-soft)] rounded-xl flex items-center justify-center mb-5">
                  ...
                  <div className="flex items-center justify-between mt-5">
                    ...
                    {!qty[i] ? (
                      <button
                        ...
                        className="px-5 py-2 rounded-full text-xs font-medium border border-[var(--accent)] text-[var(--accent)] hover:bg-[var(--accent)] hover:text-white transition"
                      >
                        Add to Cart
                      </button>
                    ) : (
