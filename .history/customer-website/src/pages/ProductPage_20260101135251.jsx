...
function VariantDropdown({ item, selected, setSelected, title }) {
    ...
    return (
        <div className="relative w-full">
            <label className="block text-xs font-medium text-[var(--text-main)] mb-1">{title}</label>
            <button
            ...
            <AnimatePresence>
                {open && (
                    <motion.ul
                        ...
                        exit={{ opacity: 0, y: -5 }}
                        className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-2xl shadow-lg overflow-hidden max-h-56 overflow-y-auto"
                    >
                            ...
                            <li
                                ...
                                className={`px-4 py-2 text-sm cursor-pointer transition ${
                                    selected === opt
                                        ...
                                        : 'text-[var(--text-main)] hover:bg-[var(--accent)] hover:text-white'
                                }`}
                            >
                                {opt} {item.variants[opt] ? `- ₹${item.variants[opt].toLocaleString('en-IN')}` : ''}
                            </li>
                        ))}
                    </motion.ul>
                )}
