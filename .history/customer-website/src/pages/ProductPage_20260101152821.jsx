  ...
  return (
    <div className="min-h-screen font-sans">
      ...
      <section className="py-16 bg-[var(--bg-soft)] text-center">
        <motion.h1 className="text-4xl sm:text-5xl font-extrabold text-[var(--text-main)]" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          Marvel Crunch Chikki
        </motion.h1>
        <motion.p className="mt-4 text-[var(--text-muted)] text-sm sm:text-base" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-4">
            ...
            <div className="mt-10 flex justify-center gap-2 flex-wrap">
              <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="px-3 py-1 rounded border bg-white hover:bg-[var(--accent)] hover:text-white transition disabled:opacity-50">Prev</button>
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i} onClick={() => goToPage(i + 1)} className={`px-3 py-1 rounded border transition ${currentPage === i + 1 ? 'bg-[var(--accent)] text-white' : 'bg-white hover:bg-[var(--accent)] hover:text-white'}`}>{i + 1}</button>
              ))}
              <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 rounded border bg-white hover:bg-[var(--accent)] hover:text-white transition disabled:opacity-50">Next</button>
            </div>
