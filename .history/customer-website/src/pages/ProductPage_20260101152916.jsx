...
<div className="mt-10 flex justify-center gap-2 flex-wrap">
  ...
  <button
    ...
    className="px-3 py-1 rounded border bg-white hover:bg-[var(--accent)] hover:text-white transition disabled:opacity-50"
  >
    Prev
  </button>
  ...
  {Array.from({ length: totalPages }, (_, i) => i + 1)
    .filter(page => {
      // Show first 2 pages, last 2 pages, current ±1 pages
      return (
        page <= 2 ||
        page > totalPages - 2 ||
    ...
    .map((page, idx, arr) => {
      ...
      return (
        <button
          ...
          className={`px-3 py-1 rounded border transition ${currentPage === page ? 'bg-[var(--accent)] text-white' : 'bg-white hover:bg-[var(--accent)] hover:text-white'}`}
        >
          {page}
        </button>
      );
