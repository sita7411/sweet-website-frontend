...
export default function ProductPage() {
    ...
    const filteredProducts = products.filter((p) => {
        ...
        const selectedWeightValue = selectedVariant[p.id] || Object.keys(p.variants)[0];
        ...
        <div className="min-h-screen font-sans">
            ...
            <section className="relative h-[70vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center mb-10">
                    ...
                    >
                        <Link to="/" className="text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors duration-300">Home</Link>
                        <span className="text-[var(--text-muted)] font-bold">\\</span>
                        <Link to="/products" className="text-[var(--text-muted)] font-semibold hover:text-[var(--text-main)] transition-colors duration-300">Products</Link>
                    </motion.div>
                </div>
            </section>
                ...
                <div className="max-w-7xl mx-auto px-4">
                    ...
                    <div className="lg:hidden flex justify-between items-center mb-6">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-3 bg-[var(--bg-card)] rounded-2xl shadow-sm border border-gray-300 text-sm font-medium"
                        >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        <aside className="hidden lg:block lg:col-span-1 sticky top-20 space-y-6">
                            ...
                            <div>
                                ...
                                <ul className="space-y-3 text-sm text-[var(--text-muted)]">
                                    {categories.filter(c => c !== 'All').map((cat, i) => (
                                        <li key={i} onClick={() => { setActiveCategory(cat); handleFilterClick(); }} className={`flex justify-between cursor-pointer hover:text-[var(--text-main)] ${activeCategory === cat ? 'font-semibold text-[var(--text-main)]' : ''}`}>
                                            <span>{cat}</span>
                            ...
                            <div>
                                ...
                                <ul className="space-y-3 text-sm text-[var(--text-muted)]">
                                    {weights.filter(w => w !== 'All').map((w, i) => (
                                        <li key={i} onClick={() => { setActiveWeight(w); handleFilterClick(); }} className={`cursor-pointer hover:text-[var(--text-main)] ${activeWeight === w ? 'font-semibold text-[var(--text-main)]' : ''}`}>
                                            {w}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full"
                                />
                                <p className="text-sm text-[var(--text-muted)] mt-2">Max Price: {formatPrice(maxPrice)}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {tags.map((t, i) => (
                                        <span key={i} onClick={() => { setActiveCategory(t); handleFilterClick(); }} className="px-3 py-1 text-xs border rounded hover:bg-[var(--accent)] hover:text-white cursor-pointer">
                                            {t}
                                        </span>
                                    ))}
                                </div>
                            ...
                            {isFilterOpen && (
                                <>
                                    ...
                                    <motion.aside
                                        ...
                                        className="fixed top-16 left-0 bottom-0 z-50 w-80 bg-[var(--bg-card)] shadow-2xl overflow-y-auto px-6 py-8 lg:hidden"
                                    >
                                        <div className="flex justify-between items-center mb-6">
                                            <h2 className="text-xl font-bold">Filters</h2>
                                            <button onClick={() => setIsFilterOpen(false)} className="p-2 rounded-full bg-[var(--bg-soft)]">
                                        ...
                                        <div className="space-y-6">
                                            <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full border px-4 py-2 rounded-md text-sm" />
                                            {/* Categories, Weight, Price, Top Products, Tags - same as desktop */}
                                            {/* You can copy the same blocks here or extract to component */}
                                            {/* For simplicity, keeping it minimal but functional */}
                                        </div>
                                    </motion.aside>
                                </>
                            )}
                        ...
                        <div className="lg:col-span-4">
                            {/* Desktop Top Bar */}
                            <div className="hidden lg:flex justify-between items-center mb-6 gap-2">
                                <p className="text-sm text-[var(--text-muted)]">Showing {(currentPage - 1) * PRODUCTS_PER_PAGE + 1}-{Math.min(currentPage * PRODUCTS_PER_PAGE, sortedProducts.length)} of {sortedProducts.length} results</p>
                            ...
                            {/* Grid - Mobile: 1 card, Tablet: 2, Desktop: 3 */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                                {displayedProducts.length === 0 && <p className="col-span-full text-center text-[var(--text-muted)] mt-10 font-semibold">No products found for selected filters.</p>}
                            ...
                            <div className="mt-12 flex justify-center items-center gap-2 text-sm font-medium">
                                <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="flex items-center gap-1 px-3 py-2 rounded-md text-[var(--text-muted)] hover:text-[var(--accent)] disabled:opacity-40 disabled:cursor-not-allowed">
                                    ‹ Previous
                                </button>

                                {Array.from({ length: totalPages }, (_, i) => i + 1)
                                    .filter(page => page <= 4 || page > totalPages - 2 || Math.abs(page - currentPage) <= 1)
                                    .map((page, idx, arr) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-9 h-9 rounded-md transition ${currentPage === page ? 'bg-[var(--accent)] text-white shadow' : 'text-[var(--text-main)] hover:bg-[var(--bg-soft)]'}`}
                                            >
                                                {page}
                                            </button>
                                        );
                                ...
                                <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className="flex items-center gap-1 px-3 py-2 rounded-md text-[var(--accent)] hover:text-[var(--primary)] disabled:opacity-40 disabled:cursor-not-allowed">
                                    Next ›
                                </button>
                            </div>
                        </div>
