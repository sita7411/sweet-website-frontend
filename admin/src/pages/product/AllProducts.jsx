...
export default function ProductPage() {
    ...
    const [itemsPerPage, setItemsPerPage] = useState(3); // dynamic
  ...
  p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6">
            {/* Toast */}
            <ToastContainer
                position="top-right"
                autoClose={2500}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                {stats.map((stat, i) => (
                    <div
                        key={i}
                        className="bg-white rounded-xl p-5 border border-[#fff1db] shadow hover:shadow-lg transition flex flex-col justify-between"
                    >
            ...
            <div className="bg-white rounded-xl border border-[#fff1db] shadow overflow-hidden">
                ...
                <div className="p-4 flex flex-col lg:flex-row justify-between gap-3 border-b border-[#fff1db]">
                    <div className="relative w-full lg:w-72">
                        <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8a6a52]" />
                        <input
                            value={searchTerm}
                            onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }}
                            placeholder="Search products..."
                            className="w-full pl-10 pr-3 py-2 text-sm sm:text-base
  rounded-lg bg-[#fff1db]"                        />
                    </div>
                </div>
                    ...
                    <table className="min-w-full text-sm">
                        ...
                        <tbody>
                            {paginatedProducts.map((p) => (
                                <tr key={p.id} className="border-t hover:bg-[#fffaf3] transition">
                                    <td className="p-3">
                                        <input
                                            type="checkbox"
                                        ...
                                        <label className="inline-flex items-center cursor-pointer">
                                            <input
                                                ...
                                                onChange={() => {
                                                    setProducts(prev =>
                                                        prev.map(prod =>
                                                            prod.id === p.id ? { ...prod, active: !prod.active } : prod
                                                        )
                                                    );
                                                }}
                                            />
    ...
    bg-gray-300 peer-checked:bg-[#c63b2f]
    ...
    " />
                                    ...
                                    <td className="p-3">
                                        <div className="flex justify-center gap-2">
                                            <button onClick={handleEdit} className="p-2 rounded-lg hover:bg-[#fff1db]">
                                                <PencilSquareIcon className="w-5 h-5 text-[#c63b2f]" />
                                            </button>
                                            <button onClick={handleDelete} className="p-2 rounded-lg hover:bg-red-50">
                    ...
                    {paginatedProducts.map((p) => (
                        <div key={p.id} className="p-4 space-y-3">
                            <div className="flex gap-3">
                                <img src={p.img} className="w-14 h-14 rounded-lg border" />
                                <div>
...
">
                        <button
                            ...
                            className="p-2 rounded-lg border bg-white hover:bg-[#fff1db] disabled:opacity-50"
                        >
...
">
                        ...
                        <button
                            ...
                            className="px-3 py-1 bg-[#c63b2f] text-white rounded-lg"
                        >
                            Go
                        </button>
                    </div>
