...
export default function ProductPage() {
    ...
    const { addToCart, cartItems, updateQty, toggleWishlist, isInWishlist } = useShop();
    ...
    // Fallback categories
    const fallbackCategories = ['All', 'Bestseller', 'Seasonal', 'Healthy', 'Premium', 'New'];
    ...
    useEffect(() => {
        async function fetchProducts() {
            try {
                ...
                const allPrices = data.flatMap((p) => p.weights?.map((w) => w.sellingPrice || 0) || []);
    ...
    const categories = dynamicCategories.length > 2 ? dynamicCategories : fallbackCategories;
    ...
    const filteredProducts = products.filter((p) => {
        ...
        const hasWeight = activeWeight === 'All' || p.weights?.some((w) => w.weight === activeWeight);
        const hasAffordableVariant = p.weights?.some((w) => (w.sellingPrice || 0) <= maxPrice);
        return catMatch && searchMatch && hasWeight && hasAffordableVariant;
    });

    // Sorting
    const sortedProducts = [...filteredProducts].sort((a, b) => {
        const getMinPrice = (p) => Math.min(...(p.weights?.map((w) => w.sellingPrice || 0) || [Infinity]));
    ...
    const SidebarContent = () => {
        const topProducts = products
            .filter((p) => p.category === 'Bestseller' || (p.tags && p.tags.includes('Bestseller')))
            .slice(0, 3);
            ...
            <div className="space-y-8">
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full"
                    />
                    <p className="text-sm text-[var(--text-muted)] mt-2">Up to {formatPrice(maxPrice)}</p>
                </div>
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        {topProducts.map((p) => {
                            const minPrice = Math.min(...(p.weights?.map((w) => w.sellingPrice) || [0]));
                            return (
                                <div key={p._id} className="flex gap-3 items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-[var(--text-muted)]">{formatPrice(minPrice)}</p>
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {['Bestseller', 'Seasonal', 'Healthy', 'Premium', 'New'].map((t) => (
                            <span
                                key={t}
                                onClick={() => {
                                    setActiveCategory(t);
                        ...
                        >
                            {variants.map((v) => (
                                <li
                                    ...
                                    className={`px-4 py-2 text-sm cursor-pointer transition flex justify-between ${selected === v.weight ? 'bg-[var(--bg-soft)] font-semibold' : 'hover:bg-[var(--bg-soft)]'
                                        }`}
                                >
                                    <span>{v.weight}</span>
                ...
                <div className="max-w-7xl mx-auto px-4">
                    ...
                    <div className="lg:hidden flex justify-between items-center mb-6">
                        <p className="text-sm text-[var(--text-muted)]">
                            ...
                            {Math.min(currentPage * PRODUCTS_PER_PAGE, sortedProducts.length)} of {sortedProducts.length} results
                        </p>
                        <button
                        ...
                        <div className="lg:col-span-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                                {displayedProducts.map((item, i) => {
                                    const id = item._id;
                                    const selectedWeight = selectedVariant[id] || item.weights?.[0]?.weight || null;
                                    const variant = item.weights?.find((w) => w.weight === selectedWeight) || item.weights?.[0] || {};
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <motion.button
                                                ...
                                                className={`absolute top-4 right-4 z-30 w-9 h-9 rounded-full flex items-center justify-center shadow-md ring-1 ring-black/10 transition-colors duration-200 ${isInWishlist(id)
                                            ...
                                            >
                                                ...
                                                <VariantDropdown
                                                    ...
                                                    onSelect={(val) =>
                                                        setSelectedVariant((prev) => ({ ...prev, [id]: val }))
                                                    }
                                                />
                                                    ...
                                                    {cartQty === 0 ? (
                                                        <button
                                                            onClick={(e) => {
                                                                ...
                                                                toast.success(`${item.name} (${variant.weight}) added to cart!`, {
                                                                    position: "top-right",
                                                                    autoClose: 2000,
                                                                    theme: "light",
                                                                });
                                                            ...
                                                            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition ${selectedWeight
                                                        ...
                                                        <div className="flex items-center gap-4 bg-[var(--bg-main)] px-5 py-2 rounded-full shadow-inner">
                                                            <button
                                                                onClick={(e) => {
                                                                    ...
                                                                    if (newQty > 0) {
                                                                        toast.info(`Updated to ${newQty}`, { autoClose: 1200 });
                                                                    } else {
                            ...
                            {totalPages > 1 && (
                                <div className="mt-12 flex justify-center items-center gap-2 text-sm font-medium">
                                    ...
                                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                                        .filter(
                                            (p) =>
                                                p <= 4 || p > totalPages - 3 || Math.abs(p - currentPage) <= 1
                                        )
                                        .map((p, idx, arr) =>
                                            idx > 0 && p - arr[idx - 1] > 1 ? (
